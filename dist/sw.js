...
const MAP_CACHE = `story-app-map-${VERSION}`;
        ...
        Promise.all([
            caches.open(STATIC_CACHE).then((cache) => {
                ...
                return cache.addAll(staticAssets).catch((error) => {
                    ...
                    // Lanjutkan install meskipun ada yang gagal
                    return Promise.resolve();
                });
            }),
            caches.open(API_CACHE).then(() => console.log("📦 API cache created")),
            caches.open(MAP_CACHE).then(() => console.log("🗺️ Map cache created"))
        ])
    );
    self.skipWaiting();
});
...
self.addEventListener("fetch", (event) => {
    ...
    // Cache-first untuk tile maps (OpenStreetMap & OpenTopoMap)
    if (
        ...
        event.respondWith(
            caches.match(event.request).then((cachedRes) => {
                ...
                return fetch(event.request).then((response) => {
                    ...
                    // Fallback untuk offline - return placeholder tile
                    return new Response(
                        '<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256"><rect fill="#f0f0f0" width="256" height="256"/></svg>',
    ...
    // Cache-first untuk aset statis (termasuk Leaflet assets)
    event.respondWith(
        caches.match(event.request).then((cacheRes) => {
            return (
                ...
                fetch(event.request).then((response) => {
                    if (response && response.status === 200) {
                        const resToCache = response.clone();
                        const cacheName = url.hostname.includes("unpkg.com")
                            ? STATIC_CACHE
                            : STATIC_CACHE;
                        caches.open(cacheName).then((cache) => cache.put(event.request, resToCache));
                    }
                    return response;
                }).catch(() => {
                    // Fallback untuk images offline
                    if (event.request.destination === 'image') {
                        return new Response(
                            '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect fill="#ddd" width="100" height="100"/><text x="50%" y="50%" text-anchor="middle" fill="#999" font-size="12">Offline</text></svg>',
                            { headers: { 'Content-Type': 'image/svg+xml' } }
                        );
                    }
                    return cacheRes;
